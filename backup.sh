...
QDATE=`/bin/date --date='-6 month' +%Y-%m-%d`
...
echo "mongoexport --authenticationDatabase $DBAUTH -u $DBUSER -p $DBPASS -h $HOST:$PORT -d $DBNAME -o ./sensorReadings.csv --collection=sensorReadings --type=csv --fields=_id,serialNumber,timeStamp,VOC,CO2,SPM1,SPM25,SPM10,AEC1,AEC25,AEC10
mongoexport --authenticationDatabase $DBAUTH -u $DBUSER -p $DBPASS -h $HOST:$PORT -d $DBNAME -o $DEST/sensorReadings.csv --collection=sensorReadings --type=csv --fields=_id,serialNumber,timeStamp,VOC,CO2,SPM1,SPM25,SPM10,AEC1,AEC25,AEC10
